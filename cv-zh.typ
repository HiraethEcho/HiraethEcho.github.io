// ZH detailed CV — cv-soft-and-hard template.
...
#show: styling
#set text(font: ("Liberation Serif", "LXGW Neo XiHei"), size:12pt)

= 王延泽 - 履历
  中国科学院数学与系统科学研究院
  基础数学博士生 预计 2027 年 6 月毕业\
  #link("mailto:wangyanze@amss.ac.cn")[wangyanze\@amss.ac.cn] | 155 3825 5707 |
...
#section("研究与实践")
  ...
  [
    ...
    - *AI 实践：* 本地 PyTorch / libtorch 小项目；智能体框架搭建。
    - *AI4Math：* AI 生成数学结果的人工验证；研究 harness与 Lean 4 形式化。
    - *工程：* torch (Python & C++); Rust, C, LaTeX; 熟悉 Linux 和 Git；个人使用的 CLI / TUI 程序以 vibe coding 开发（Rust）
  ],
  [],
)
...
#subsection("编程")
#entry(
  [#link("https://github.com/hiraethecho/suckless")[*suckless*] — 自行 fork 的 dwm / dwmblocks（X11 动态平铺窗口管理器，C），手工补丁维护，未经 AI 辅助。],
  [],
)
#entry(
  [#link("https://github.com/hiraethecho/calman")[*Calman*] — 终端任务与事件管理器（CLI + TUI，Rust）。JSONL / ICS 存储，兼容 CalDAV，同步委托 git / vdirsyncer / rclone。],
  [],
)
#entry(
  [#link("https://github.com/hiraethecho/markerss")[*Markerss*] — TUI RSS 阅读器（Rust，ratatui + feed-rs）：Markdown 导出、仅存 URL 收藏。],
  [],
)
#entry(
  [#link("https://github.com/hiraethecho/lichtung")[*Lichtung*] — Hugo 主题（Go 模板 / SCSS），以对话式 AI 辅助开发完成，支撑两个个人站点。],
  [],
)
#entry(
  [#link("https://github.com/hiraethecho/mumble")[*mumble*] — 留言板（JavaScript，Cloudflare Worker），可独立部署或嵌入页面。],
  [],
)
...
#entry(
  [#link("https://github.com/hiraethecho/AiKit")[*AiKit*] — 智能体框架：面向编码智能体（pi、opencode、codex、claude code 等）的代理 / skills / 插件集合，含 pi 扩展（rtk、cave、toon、doc、role）与快速部署脚本。],
  [],
)
#entry(
